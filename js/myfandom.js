...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  function drawHome(){
    ...
    const cards = [
      ...
      ...ev.map(e => `<a class="feed-card" href="events.html">${e.img ? thumb(`<img src="${e.img}" alt="" loading="lazy">`) : e.art ? thumb(FV.slotImg("articles", e.art, e.title)) : ""}${tag(e.category)}<span class="feed-kind">Upcoming event · ${new Date(e.date).toLocaleDateString(undefined, { day: "numeric", month: "short" })}</span><strong>${esc(e.title)}</strong></a>`),
